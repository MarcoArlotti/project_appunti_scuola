...
#        if utente:
#            password_messa = utente.password
#            if check_password_hash(password_messa, password):
...
from flask import Blueprint, flash, redirect, render_template, request, url_for, session
from app.repositories.crud import *
from app.models import Note  # Importato per la gestione diretta del modello nella cancellazione
from datetime import datetime
...
@bp.route("/", methods=["GET", "POST"])
def index():
    # Ottieni l'username dalla sessione in modo sicuro
...
def login():
    if request.method == "POST":
        ...
        # controlla_accesso verifica già l'hash internamente e restituisce l'oggetto Student (o None)
        utente = controlla_accesso(username, password)
        if utente:
            session['id'] = utente.id
            session['username'] = utente.username
            ...
            flash("Utente non esiste o password non valida")
...
@bp.route("/create_notes", methods=["GET", "POST"])
def c_note():
    # Verifica robusta della sessione (controlla che ci sia l'ID)
    ...
    if request.method == "POST":
        ...
        data_upload = datetime.utcnow() # Consigliato UTC per coerenza con i default del database
        student_id = session["id"]
        subject_id = request.form.get('subject')
        
        id_post_creato = crea_note(text_data, title, data_upload, student_id, subject_id)
        return redirect(url_for("main.specific_note", id=id_post_creato))
...
def delete_post(id):
    if 'id' in session:
        # Recuperiamo l'oggetto Note effettivo dal database usando SQLAlchemy direttamente
        # per passarlo a cancella_post(note, session)
        note_da_cancellare = Note.query.get(id)
        
        if note_da_cancellare:
            cancella_post(note_da_cancellare, session)
